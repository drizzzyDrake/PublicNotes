In RISC-V, i **CSR (Control and Status Registers)** sono registri speciali **interni alla CPU**, usati per: controllare il comportamento del processore (status, interrupt, ecc.), gestire il passaggio tra [[RISC-V Privilege Levels|modalità di privilegio]] (U/S/M), **osservare o configurare** lo stato della CPU.
